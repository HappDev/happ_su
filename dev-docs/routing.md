...
В приложении реализован переход от монолитного глобального управления к модульному управлению на уровне подписок:
* **Индивидуальный набор правил:** Теперь у каждой подписки (включая список серверов Server-List) есть собственный изолированный набор правил и профилей маршрутизации («Пульт управления»), имеющий независимый жизненный цикл.
* **Отложенный эффект изменений:** Изменение любых настроек (переключение тумблера маршрутизации, выбор другого профиля) записывается в БД, но применяется только после перезапуска соединения (**Reconnect**).
* **Каскадное удаление:** При удалении подписки из приложения каскадно удаляются все привязанные к ней профили маршрутизации и соответствующие кэшированные геофайлы на диске.

#### **Особенности работы с JSON-подписками (Restricted Mode)**

Для предотвращения рассинхронизации настроек приложения с метаданными провайдера для JSON-конфигураций действует режим жестких ограничений:
* **Лимит профилей:** У JSON-подписки может существовать строго **0 или 1** профиль маршрутизации.
* **Только в комплекте с подпиской:** Профиль маршрутизации для JSON-конфигурации не может быть добавлен вручную, скопирован из другой подписки или импортирован из буфера обмена. Он приходит **исключительно вместе с самой подпиской** от провайдера. Возможность ручного создания, копирования полностью заблокированы.
* **Невозможность отключить или изменить:** Пользователь лишен возможности вручную управлять активностью такого профиля или менять его параметры:
  * Главный тогл маршрутизации принудительно заблокирован в состоянии `true` (или `false`, если отключение прислал сам провайдер).
  * Изменение имени, ссылок и самих геофайлов (блоков GeoSite/GeoIP) полностью заблокировано.
* **Tunnel DNS:** Поля Remote DNS в интерфейсе переименованы в **Tunnel DNS** (Tunnel DNS Type, Tunnel IP, Tunnel Domain) и используются локально для настройки системного туннеля, не инжектируясь в сам JSON-файл провайдера.
...
При добавлении профиля из хедеров или тела - сразу привязывает роутинг профиль к подписке в которой он пришёл.
...
#### Добавление / обновление профиля

**Добавление профиля маршрутизации**

При импорте профиля роутинга, если такового не было в подписке, то профиль добавляется и привязывается к этой подписке и происходит скачивание геофайлов.

**Обновление профиля маршрутизации**

Процесс обновления запускается, когда в систему поступает команда на импорт профиля с уже существующим именем.

* **Инициация обновления.** Система получает новый импорт профиля. Сущность переходит в транзитное состояние Update.
* **Фоновая загрузка (Сохранение работоспособности).** Начинается процесс скачивания новых геофайлов по ссылкам из обновленной версии.
  * **Важно:** В процессе скачивания состояние Current остается неизменным. Ядро продолжаем запускать со старыми правила и старыми геофайлы, обеспечивая бесперебойную маршрутизацию.
* **Завершение загрузки и атомарная подмена.** Как только все геофайлы для обновленной версии успешно докачались:
  * Происходит замена старых геофайлов на новые.
  * Стейт Current перезаписывается новыми правилами из импорта.
* **Синхронизация состояний.** После успешной подмены система обновляет метаданные внутри единой сущности:
  * Стейт Default обновляется — теперь дефолтным считается тот набор правил, который только что был импортирован.
  * Стейт Update помечается как завершенный (или очищается), и система возвращается в штатный режим работы.
