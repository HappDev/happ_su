...
{% hint style="info" %}
Чтобы отключить объявление, активированное через HTTP-заголовок или push, отправьте значение 0.
{% endhint %}
...
| sub-info-text          | String  | Yes\*    | max 200 chars                   | Основной текст инфо-блока. Для отключения - параметр отправте: 0.  |
...
* **via Proxy - HEAD**
* **TCP**
* **ICMP** Для режима «via Proxy» можно дополнительно указать URL для проверки пинга.
...
* Если в имени есть "only Mobile" — он виден только при мобильном интернете. Данная опция отключает это поведение.
